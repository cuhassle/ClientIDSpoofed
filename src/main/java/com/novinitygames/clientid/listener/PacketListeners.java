...
public class PacketListeners {
    public static void Register() {
        ServerPlayNetworking.registerGlobalReceiver(ModCheckC2SPayload.ID, (payload, player, sender) -> {
            ClientID.modConfirmed.put(player, true);
        });

        ServerPlayNetworking.registerGlobalReceiver(ModListC2SPayload.ID, (payload, player, sender) -> {
            ...
            if (!bannedMods.isEmpty()) {
                ...
                player.networkHandler.disconnect(Text.literal(kickMessage.toString()).styled(style -> style.withColor(0xFF5555)));
            }
        });

        ServerPlayNetworking.registerGlobalReceiver(PackListC2SPayload.ID, (payload, player, sender) -> {
            String s = payload.list();
        ...
        ServerPlayNetworking.registerGlobalReceiver(VersionC2SPayload.ID, (payload, player, sender) -> {
            if (ClientID.isPlayerBypassed(player)) return;
            ...
            try {
                ...
                if (ClientID.clientMinimumVersion.compareTo(version) > 0) {
                    player.networkHandler.disconnect(Text.literal("The minimum version of ClientID required to join this server is "
                            + ClientID.clientMinimumVersion.toString()).styled(style -> style.withColor(0xFF5555)));
                } else {
                    ClientID.versionConfirmed.put(player, true);
                }
            } catch (Exception e) {
                player.networkHandler.disconnect(Text.literal("Failed to receive ClientID version.").styled(style -> style.withColor(0xFF5555)));
            }
        });
    }
}
