...
public class ModCommandManager {
        ...
        CommandRegistrationCallback.EVENT.register(
            ...
            dispatcher.register(CommandManager.literal("clientid").executes(context -> {
                FabricLoader.getInstance().getModContainer("clientid").ifPresent(modContainer -> {
                    ...
                    context.getSource().sendMessage(Text.literal("[ClientID] Version " + version).styled(style -> style.withColor(0x55FF55)));
                });
                return 1;
            })
                    .then(CommandManager.literal("reload").executes(ModCommandManager::ReloadSubCommand).requires(source -> source.hasPermissionLevel(4)))
    ...
    private static int ReloadSubCommand(CommandContext<ServerCommandSource> ctx) {
        ...
        for (ServerPlayerEntity player : ctx.getSource().getServer().getPlayerManager().getPlayerList()) {
            player.networkHandler.disconnect(Text.literal("Config reloaded. Please rejoin.").styled(style -> style.withColor(0xFF5555)));
        }
    ...
    private static int ViewModsSubCommand(CommandContext<ServerCommandSource> ctx) {
        ...
        if (target == null) {
            ctx.getSource().sendMessage(Text.literal("Player not found.").styled(style -> style.withColor(0xFF5555)));
            return 0;
        }

        ctx.getSource().sendMessage(Text.literal(target.getName().getString() + "'s mod list:").styled(style -> style.withColor(0xFFAA00)));
        ctx.getSource().sendMessage(Text.literal(String.join(", ", ClientID.modLists.get(target))).styled(style -> style.withColor(0x55FF55)));
    ...
    private static int ViewPacksSubCommand(CommandContext<ServerCommandSource> ctx) {
        ...
        ctx.getSource().sendMessage(Text.literal(target.getName().getString() + "'s pack list:").styled(style -> style.withColor(0xFFAA00)));
        ctx.getSource().sendMessage(Text.literal(String.join(", ", ClientID.packLists.get(target))).styled(style -> style.withColor(0x55FF55)));
